...
def main(local_rank, args):
    ...
    timestamp = now.strftime('%Y%m%d_%H%M%S')  # 예: 2025-04-15_14-30-00
    ...
    writer = None
    if local_rank == 0:
        writer = SummaryWriter(log_dir=osp.join(args.work_dir, f'tf_logs/{timestamp}'))
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (input_occs, target_occs, metas) in enumerate(train_dataset_loader):
            ...
            if i_iter % print_freq == 0 and local_rank == 0:
                ...
                # logger.info('[TRAIN] usages : %s' % (result_dict['usages']))
                detailed_loss = []
        ...
        with torch.no_grad():
            for i_iter_val, (input_occs, target_occs, metas) in enumerate(val_dataset_loader):
                ...
                # plan_loss += loss_dict.get('PlanRegLossLidar_t', 0)
                # plan_loss += loss_dict.get('PlanRegLossLidar_m', 0)
                # plan_loss += loss_dict.get('PlanRegLossLidar_b', 0)
        ...
        if (np.sum(val_iou) > np.sum(best_val_iou)) and (np.sum(val_miou) > np.sum(best_val_miou)):
            best_val_iou = [val_iou[i] for i in range(len(best_val_iou))]
            best_val_miou = [val_miou[i] for i in range(len(best_val_miou))]
        ...
        if local_rank == 0:
            if writer is not None:
                writer.add_scalar(f'VAL/IoU', np.mean(val_iou), epoch)
                writer.add_scalar(f'VAL/mIoU', np.mean(val_miou), epoch)
                writer.add_scalar(f'VAL/BEST_IoU', np.mean(best_val_iou), epoch)
                writer.add_scalar(f'VAL/BEST_mIoU', np.mean(best_val_miou), epoch)
