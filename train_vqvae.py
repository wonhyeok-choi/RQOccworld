...
def main(local_rank, args):
    ...
    if args.gpus > 1:
        ...
        port = os.environ.get("MASTER_PORT", cfg.get("port", 29504))
    ...
    while epoch < max_num_epochs:
        ...
        #logger.info(f'PlanRegLoss is {plan_loss/len(val_dataset_loader)}')
        logger.info(f'Current val iou is {val_iou} while the best val iou is {val_iou}')
        logger.info(f'Current val miou is {val_miou} while the best val miou is {val_miou}')
        if local_rank == 0:
            if writer is not None:
