...
class TransRQVAE(BaseModule):
    ...
    def __init__(self, vae, transformer_t, transformer_d, num_frames=10, offset=1,
        ...
        self.transformer_t = MODELS.build(transformer_t)
        self.transformer_d = MODELS.build(transformer_d)
    ...
    def forward_train_with_plan(self, x, metas):
        # assert hasattr(self.vae, 'vqvae_top')
        # assert hasattr(self.vae, 'vqvae_middle') # NOTE TODO WONHYEOK VAERes2D에 vqvae_middle 확인 -> 확인완료
        # assert hasattr(self.vae, 'vqvae_bottom')
        is_soft = True
        ...
        final_quant, residual_list, code_list, soft_code_list, shapes, out_shape = self.vae.encode(x, isSoft=is_soft) # [bs*f, H, W, 50], [bs*f, H, W]
        if is_soft:
            output_dict['ce_labels_t'] = rearrange(soft_code_list[0][self.offset:].detach(), 'bf h w c -> bf c h w') # NOTE 첫번째 depth의 next frame
            output_dict['ce_labels_d'] = rearrange(torch.cat([soft_code_list[1], soft_code_list[2], soft_code_list[3]], dim=0).detach(), 'bfd h w c -> bfd c h w') # NOTE 현재 frame의 미래 예측 (b f d-1) h w HARDCODED
        else:
            output_dict['ce_labels_t'] = code_list[0][self.offset:].detach() # NOTE 첫번째 depth의 next frame
            output_dict['ce_labels_d'] = torch.cat([code_list[1], code_list[2], code_list[3]], dim=0).detach() # NOTE 현재 frame의 미래 예측 (b f d-1) h w HARDCODED

        rel_poses_, output_metas = self._get_pose_feature(metas, f-self.offset) # sequential poses except for last time stamp
        z_q_t = rearrange(residual_list[0], '(b f) h w c -> b f c h w', b=bs, f=f)
        # z_q_d = rearrange(torch.stack(residual_list, dim=1), 'bf d h w c -> bf d c h w')
        z_q_d = rearrange(torch.cumsum(torch.stack(residual_list, dim=1), dim=1), 'bf d h w c -> bf d c h w')
        
        #############################################################
        # NOTE OPTION 1. independent causal transformer
        # shape = z_q_m[:, :self.num_frames].shape # (b f' 128 25 25) # TODO 위에 먼저 선언된 shape override 해제

        z_q_t, rel_poses_b = self.transformer_t( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                                    z_q_t[:, :self.num_frames],
                                    rel_poses_
                                ) # b f c h w
        z_q_d = self.transformer_d(
                                    z_q_d[:, :self.depth-1],
                                ) # bf d c h w
        #############################################################
        
        output_dict['ce_inputs_t'] = z_q_t.flatten(0, 1) # bf c h w
        output_dict['ce_inputs_d'] = z_q_d.flatten(0, 1) # bfd c h w
        # print("ce_inputs_t:", output_dict['ce_inputs_t'].shape)
        # print("ce_inputs_d:", output_dict['ce_inputs_d'].shape)
        # output_dict['ce_inputs_b'] = z_q_b.flatten(0, 1)

        pose_decoded = self.pose_decoder(rel_poses_b) # [bs, f-1, 3, 2]
    ...
    def forward_inference_with_plan(self, x, metas):
        ...
        final_quant, residual_list, code_list, soft_code_list, shapes, out_shape = self.vae.encode(x, isSoft=True) # [bs*f, H, W, 50], [bs*f, H, W]
        output_dict['ce_labels_t'] = code_list[0][self.offset:].detach() # NOTE 첫번째 depth의 next frame
        output_dict['ce_labels_d'] = torch.cat([code_list[1][self.offset:], code_list[2][self.offset:], code_list[3][self.offset:]], dim=0).detach() # NOTE 현재 frame의 미래 예측 (b f d-1) h w HARDCODED
        ...
        idx_t = torch.argmax(z_q_t.flatten(0, 1), dim=1) # b f c h w -> bf h w
        quant_t = self.vae.vqvae.embed_code(idx_t).permute(0,3,1,2).unsqueeze(1) # bf, 1, c, h, w
        z_q_d_input = quant_t

        for i in range(1, 4):
            shape_input = z_q_d_input.shape # bf 1 c h w
            z_q_d_ = self.transformer_d.forward_autoreg_step(
                z_q_d_input,
                start_frame=0, mid_frame=i)
            idx_d = torch.argmax(z_q_d_.flatten(0, 1), dim=1) # bfd h w
            quant_d = self.vae.vqvae.embed_code(idx_d).permute(0,3,1,2).view(*shape_input) # bf d c h w
            quant_t += quant_d[:, -1:]
            # z_q_d_input = torch.cat([z_q_d_input, quant_d[:, -1:]], dim=1) # bf d+1 c h w
            z_q_d_input = torch.cat([z_q_d_input, quant_t[:, -1:]], dim=1) # bf d+1 c h w
        z_q_d = z_q_d_
        #############################################################

        output_dict['ce_inputs_t'] = z_q_t.flatten(0, 1)
        output_dict['ce_inputs_d'] = z_q_d.flatten(0, 1) # NOTE ADDED
        ...
        logits = self.vae.decode(quant_t.squeeze(1).permute(0, 2, 3, 1), shapes, output_dict['target_occs'].shape)
    ...
    def forward_autoreg_with_pose(self, x, metas, start_frame=0, mid_frame=6,end_frame=12): # 0, 5, 11
        ...
        z_q_t = rearrange(z_q_t, '(b f) h w c -> b f c h w', b=bs)
        z_q_m = rearrange(z_q_m, '(b f) h w c -> b f c h w', b=bs) # NOTE ADDED
        z_q_b = rearrange(z_q_b, '(b f) h w c -> b f c h w', b=bs)
        # origin_z_q_t = z_q_t.clone()
        # origin_z_q_b = z_q_b.clone()
        z_q_t_predict = z_q_t[:, start_frame:mid_frame]
        z_q_m_predict = z_q_m[:, start_frame:mid_frame] # NOTE ADDED
        z_q_b_predict = z_q_b[:, start_frame:mid_frame]
        ...
        z_q_t_list = []
        z_q_m_list = [] # NOTE ADDED
        z_q_b_list = []
        t2 = time.time()
        poses_ = []
        for i in range(mid_frame, end_frame):
            # print(f">>>> frame number : {i}, z_q_t_predict : {z_q_t_predict.shape}, z_q_b_predict : {z_q_b_predict.shape}")
            ###########################################
            # NOTE OPTION 1. just individual transformer
            # z_q_t_, rel_poses_t_ = self.transformer_t.forward_autoreg_step(
            #     z_q_t_predict, pose_tokens=rel_poses_state,
            #     start_frame=start_frame, mid_frame=i)
            # z_q_m_, rel_poses_m_ = self.transformer_m.forward_autoreg_step(
            #     z_q_m_predict, pose_tokens=rel_poses_t,
            #     start_frame=start_frame, mid_frame=i)
            # z_q_b_, rel_poses_b_ = self.transformer_b.forward_autoreg_step(
            #     z_q_b_predict, pose_tokens=rel_poses_m,
            #     start_frame=start_frame, mid_frame=i)
            ###########################################
            # NOTE OPTION 2. residual + cascade + frame index align
            shape_m = z_q_m_predict.shape # (b f' 128 25 25)
            shape_b = z_q_b_predict.shape # (b f' 128 50 50)

            z_q_t_ = self.transformer_t.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_t_predict,
                start_frame=start_frame, mid_frame=i)
            temp_z_q_t = torch.argmax(z_q_t_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_t = self.vae.vqvae_top.embed_code(temp_z_q_t).permute(0,3,1,2) # bf, 128, 12, 12
            # next_quant_t = self.vae.vqvae_top.embed_soft_code(z_q_t_.detach().clone()).permute(0,3,1,2)
            next_t2m = F.interpolate(next_quant_t, size=(25, 25), mode='nearest').view(*shape_m) # b, f, 128, 25, 25
            """ NOTE IDEA: we will use the original z_q_m with previous frames as middle scale residual information"""
            # next_t2m[:, :-1]  = next_t2m[:, :-1] + z_q_m_predict[:, 1:]
            
            z_q_m_ = self.transformer_m.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_m_predict, guide=next_t2m,
                start_frame=start_frame, mid_frame=i)
               
            temp_z_q_m = torch.argmax(z_q_m_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_m = self.vae.vqvae_middle.embed_code(temp_z_q_m).permute(0,3,1,2) # bf, 128, 12, 12
            # next_quant_m = self.vae.vqvae_middle.embed_soft_code(z_q_m_.detach().clone()).permute(0,3,1,2)
            next_m2b = F.interpolate(next_quant_m, size=(50, 50), mode='nearest').view(*shape_b) # b, f, 128, 25, 25
            # next_m2b[:, :-1]  = next_m2b[:, :-1] + z_q_b_predict[:, 1:] # 1~forecasted frame (next t and original middle scale)

            z_q_b_, rel_poses_b_ = self.transformer_b.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_b_predict, pose_tokens=rel_poses_state, guide=next_m2b,
                start_frame=start_frame, mid_frame=i)
            #############################################################
            # NOTE OPTION 2-1. PLEASE DEBUG OPTION 2,,,
            #############################################################
            # # NOTE OPTION 3. align with original VQ-VAE2
            # shape_t = z_q_t_predict.shape # (b f' 128 12 12)
            # shape_m = z_q_m_predict.shape # (b f' 128 25 25)
            # shape_b = z_q_b_predict.shape # (b f' 128 50 50)
            # cond_t = z_q_t_predict.flatten(0, 1) # bf' 128 12 12
            # cond_m = z_q_m_predict.flatten(0, 1) # bf' 128 25 25
            # cond_b = z_q_b_predict.flatten(0, 1) # bf' 128 50 50
            # cond_t2m = F.interpolate(self.cond_resnet_t2m(cond_t), size=shape_m[-2:], mode='bilinear') # bf' 128 25 25
            # cond_t2b = F.interpolate(self.cond_resnet_t2b(cond_t), size=shape_b[-2:], mode='bilinear') # bf' 128 50 50
            # cond_m2b = F.interpolate(self.cond_resnet_m2b(cond_m), size=shape_b[-2:], mode='bilinear') # bf' 128 50 50

            # # 1. TOP SCALE: just use z_q_t solely
            # final_t = cond_t.view(*shape_t) # b f' 128 12 12
            # # print("final_t.shape:", final_t.shape) # 1 5 128 12 12
            
            # # 2. MID SCALE: fuse (top, middle) to middle
            # for block in self.fusion_tm_on_midscale:
            #     cond_m = block(cond_m, condition=cond_t2m)
            # final_m = cond_m.view(*shape_m) # b f' 128 25 25

            # # 3. BOT SCALE: fuse (top, middle, bottom) to middle
            # for block in self.fusion_tm_on_botscale:
            #     cond_m2b = block(cond_m2b, condition=cond_t2b) # tm fusion
            # for block in self.fusion_mb_on_botscale:
            #     cond_b = block(cond_b, condition=cond_m2b)
            # final_b = cond_b.view(*shape_b)

            # # print("final_m.shape:", final_m.shape) # 1 5 128 12 12
            # # print("final_b.shape:", final_b.shape) # 1 5 128 12 12

            # z_q_t_, rel_poses_t_ = self.transformer_t.forward_autoreg_step(
            #     final_t, pose_tokens=rel_poses_state,
            #     start_frame=start_frame, mid_frame=i)
            # # print("z_q_t_.shape:", z_q_t_.shape) # 1, 5, 512, 12, 12
            # z_q_m_, rel_poses_m_ = self.transformer_m.forward_autoreg_step(
            #     final_m, pose_tokens=rel_poses_state,
            #     start_frame=start_frame, mid_frame=i)
            # # print("z_q_m_.shape:", z_q_m_.shape) # 1, 5, 512, 25, 25
            # z_q_b_, rel_poses_b_ = self.transformer_b.forward_autoreg_step(
            #     final_b, pose_tokens=rel_poses_state,
            #     start_frame=start_frame, mid_frame=i)
            # # print("z_q_b_.shape:", z_q_b_.shape) # 1, 5, 512, 50, 50
            ###########################################

            z_q_t_list.append(z_q_t_[:, -1:])
            z_q_m_list.append(z_q_m_[:, -1:]) # NOTE ADDED
            ...
            z_q_t_predict = torch.cat([z_q_t_predict, z_q_t_], dim=1)
            z_q_m_predict = torch.cat([z_q_m_predict, z_q_m_], dim=1)
            z_q_b_predict = torch.cat([z_q_b_predict, z_q_b_], dim=1)
            # print("z_q_t_predict.unique:", torch.unique(z_q_t_predict))
            ##########################
        ...
        z_q_t = torch.cat(z_q_t_list, dim=1)
        z_q_m = torch.cat(z_q_m_list, dim=1)
        z_q_b = torch.cat(z_q_b_list, dim=1)
        output_dict['ce_inputs_t'] = z_q_t.flatten(0, 1)
        output_dict['ce_inputs_m'] = z_q_m.flatten(0, 1)
        output_dict['ce_inputs_b'] = z_q_b.flatten(0, 1)

        
        z_q_t_predict = z_q_t_predict.flatten(0, 1).permute(0, 2, 3, 1) 
        ...
        z_q_predict = self.vae.decode(z_q_t_predict, z_q_m_predict, z_q_b_predict, shapes_b, output_dict['target_occs'].shape)
