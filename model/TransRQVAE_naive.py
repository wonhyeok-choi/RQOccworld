...
class TransRQVAENaive(BaseModule):
    ...
    def forward_train_with_plan(self, x, metas):
        ...
        if is_soft:
            output_dict['ce_labels_1'] = rearrange(torch.stack(soft_code_list, dim=1)[self.offset:], '(b f) d h w c -> (b d f) c h w', b=bs, f=f-1).detach()
        else:
            output_dict['ce_labels_1'] = rearrange(torch.stack(code_list, dim=1)[self.offset:], '(b f) d h w -> (b d f) h w', b=bs, f=f-1).detach()

        rel_poses_, output_metas = self._get_pose_feature(metas, f-self.offset) # sequential poses except for last time stamp
        z_q_1 = rearrange(torch.stack(residual_list, dim=1), '(b f) d h w c -> (b d) f c h w', b=bs, f=f)
    ...
    def forward_inference_with_plan(self, x, metas):
        ...
        rel_poses_, output_metas = self._get_pose_feature(metas, f-self.offset) # sequential poses except for last time stamp
        z_q_1 = rearrange(torch.stack(residual_list, dim=1), '(b f) d h w c -> (b d) f c h w', b=bs, f=f)
        # output_dict['ce_labels_t'] = rearrange(torch.stack(code_list, dim=1), '(b f) d h w -> (b d f) h w', b=bs, f=f).detach()
        ...
        #                             rel_poses_
        #                         ) # bd f c h w
        idx_t = torch.argmax(z_q_1.flatten(0, 1), dim=1) # bdf h w
        quant_t = self.vae.vqvae.embed_code(idx_t) # bdf h w c
        quant_t = rearrange(quant_t, '(b d f) h w c -> (b f) d h w c', b=bs, d=self.depth, f=self.num_frames).sum(1) # bf h w c
        #############################################################
        ...
        logits = self.vae.decode(quant_t, shapes, output_dict['target_occs'].shape)
    ...
    def forward_autoreg_with_pose(self, x, metas, start_frame=0, mid_frame=6,end_frame=12): # 0, 5, 11
        ...
        final_quant, residual_list, code_list, soft_code_list, shapes, out_shape = self.vae.encode(x, isSoft=False) # [bs*f, H, W, 50], [bs*f, H, W]
        output_dict['ce_labels_1'] = rearrange(torch.stack(code_list, dim=1)[mid_frame:end_frame], '(b f) d h w -> (b d f) h w', b=bs, f=end_frame-mid_frame).detach()
        z_q_1 = rearrange(torch.stack(residual_list, dim=1), '(b f) d h w c -> (b d) f c h w', b=bs, f=f)
        ...
        for i in range(mid_frame, end_frame):
            #############################################################
            # NOTE OPTION 1. independent causal transformer
            z_q_1_, rel_poses_b_ = self.transformer_1.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_1_predict, pose_tokens=rel_poses_state,
            ...
            idx_1 = torch.argmax(z_q_1_.flatten(0, 1), dim=1)
            quant_1 = self.vae.vqvae.embed_code(idx_1) # bdf h w c
            z_q_1_ = rearrange(quant_1, '(b d f) h w c -> (b d) f c h w', b=bs, d=self.depth, f=(quant_1.shape[0]//bs)//self.depth) # (b d) f c h w
            ...
            ##########################
            # print("rel_poses:", rel_poses.shape)
            # print("b:", rel_poses_b_.shape)
            rel_poses = torch.cat([rel_poses, rel_poses_b_.mean(0, True)[:, -1:]], dim=1)
            rel_poses_state_, rel_poses_sumed, pose_ = self.decode_pose(rel_poses_b_.mean(0, True)[:, -1:], gt_mode[:,i:i+1], rel_poses_sumed)
        ...
        z_q_1_predict = z_q_1_predict.sum(0, True).flatten(0, 1).permute(0, 2, 3, 1)  # bf c h w
