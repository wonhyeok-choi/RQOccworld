...
class TransRQVAEIndiv(BaseModule):
    ...
    def __init__(self, vae, transformer_1, transformer_2, transformer_3, transformer_4, num_frames=10, offset=1,
    ...
    def forward_train_with_plan(self, x, metas):
        ...
        if is_soft:
            ...
            output_dict['ce_labels_1'] = code_list[0][self.offset:].detach()
            output_dict['ce_labels_2'] = code_list[1][self.offset:].detach()
            output_dict['ce_labels_3'] = code_list[2][self.offset:].detach()
            output_dict['ce_labels_4'] = code_list[3][self.offset:].detach()

        rel_poses_, output_metas = self._get_pose_feature(metas, f-self.offset) # sequential poses except for last time stamp
        # z_q_t = rearrange(torch.stack(residual_list, dim=1), '(b f) d h w c -> (b d) f c h w', b=bs, f=f)
        z_q_1 = rearrange(residual_list[0], '(b f) h w c -> b f c h w', b=bs, f=f)
        z_q_2 = rearrange(residual_list[1], '(b f) h w c -> b f c h w', b=bs, f=f)
        z_q_3 = rearrange(residual_list[2], '(b f) h w c -> b f c h w', b=bs, f=f)
        z_q_4 = rearrange(residual_list[3], '(b f) h w c -> b f c h w', b=bs, f=f)
        
        #############################################################
        z_shape = z_q_1[:, :self.num_frames].shape # (b f' 128 25 25) # TODO 위에 먼저 선언된 shape override 해제
        z_q_1 = self.transformer_1( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                                        z_q_1[:, :self.num_frames],
                                    ) 
        #
        idx_1 = torch.argmax(z_q_1.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_1 = self.vae.vqvae.embed_code(idx_1).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12
        
        z_q_2 = self.transformer_2( # LIGHTWEIGHT transformer NOTE already forecasted input, just need to refine to midscale
                                        z_q_2[:, :self.num_frames],
                                        guide=next_quant_1
                                    ) 
        idx_2 = torch.argmax(z_q_2.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_2 = self.vae.vqvae.embed_code(idx_2).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12
        
        z_q_3 = self.transformer_3( # LIGHTWEIGHT transformer NOTE already forecasted input, just need to refine to midscale
                                        z_q_3[:, :self.num_frames],
                                        guide=next_quant_2
                                    ) 
        idx_3 = torch.argmax(z_q_3.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_3 = self.vae.vqvae.embed_code(idx_3).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12
        
        z_q_4, rel_poses = self.transformer_4( # LIGHTWEIGHT transformer
                                        z_q_4[:, :self.num_frames],
                                        rel_poses_,
                                        guide=next_quant_3
                                    ) 
        #############################################################
        
        output_dict['ce_inputs_1'] = z_q_1.flatten(0, 1) # bdf c h w
    ...
    def forward_inference_with_plan(self, x, metas):
        ...
        final_quant, residual_list, code_list, soft_code_list, shapes, out_shape = self.vae.encode(x, isSoft=False) # [bs*f, H, W, 50], [bs*f, H, W]
        output_dict['ce_labels_1'] = code_list[0][self.offset:].detach() # bf h w
        output_dict['ce_labels_2'] = code_list[1][self.offset:].detach()
        output_dict['ce_labels_3'] = code_list[2][self.offset:].detach()
        output_dict['ce_labels_4'] = code_list[3][self.offset:].detach()
        ...
        rel_poses_, output_metas = self._get_pose_feature(metas, f-self.offset) # sequential poses except for last time stamp
        # z_q_t = rearrange(torch.stack(residual_list, dim=1), '(b f) d h w c -> (b d) f c h w', b=bs, f=f)
        z_q_1 = rearrange(residual_list[0], '(b f) h w c -> b f c h w', b=bs, f=f) # b f c h w
        z_q_2 = rearrange(residual_list[1], '(b f) h w c -> b f c h w', b=bs, f=f)
        z_q_3 = rearrange(residual_list[2], '(b f) h w c -> b f c h w', b=bs, f=f)
        z_q_4 = rearrange(residual_list[3], '(b f) h w c -> b f c h w', b=bs, f=f)
        ...
        #
        idx_1 = torch.argmax(z_q_1.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_1 = self.vae.vqvae.embed_code(idx_1).permute(0,3,1,2).view(*z_shape) # b f, 128, 12, 12
        ...
        idx_2 = torch.argmax(z_q_2.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_2 = self.vae.vqvae.embed_code(idx_2).permute(0,3,1,2).view(*z_shape) # b f, 128, 12, 12
        ...
        idx_3 = torch.argmax(z_q_3.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_3 = self.vae.vqvae.embed_code(idx_3).permute(0,3,1,2).view(*z_shape) # b f, 128, 12, 12
        ...
        idx_4 = torch.argmax(z_q_4.flatten(0, 1), dim=1) # bf c h w -> b f h w
        next_quant_4 = self.vae.vqvae.embed_code(idx_4).permute(0,3,1,2).view(*z_shape) # b f, 128, 12, 12 

        quant_final = (next_quant_1 + next_quant_2 + next_quant_3 + next_quant_4).flatten(0, 1).permute(0,2,3,1)
        ...
        # NOTE TODO WONHYEOK: vae.decode_code midscale process 추가 필요
    ...
    def forward_autoreg_with_pose(self, x, metas, start_frame=0, mid_frame=6,end_frame=12): # 0, 5, 11
        ...
        output_dict['ce_labels_1'] = code_list[0][mid_frame:end_frame].detach()
        output_dict['ce_labels_2'] = code_list[1][mid_frame:end_frame].detach()
        output_dict['ce_labels_3'] = code_list[2][mid_frame:end_frame].detach()
        output_dict['ce_labels_4'] = code_list[3][mid_frame:end_frame].detach()

        z_q_1 = rearrange(residual_list[0], '(b f) h w c -> b f c h w', b=bs)
        z_q_2 = rearrange(residual_list[1], '(b f) h w c -> b f c h w', b=bs)
        z_q_3 = rearrange(residual_list[2], '(b f) h w c -> b f c h w', b=bs)
        z_q_4 = rearrange(residual_list[3], '(b f) h w c -> b f c h w', b=bs)
        # origin_z_q_t = z_q_t.clone()
        # origin_z_q_b = z_q_b.clone()
        z_q_1_predict = z_q_1[:, start_frame:mid_frame]
        z_q_2_predict = z_q_2[:, start_frame:mid_frame] # NOTE ADDED
        z_q_3_predict = z_q_3[:, start_frame:mid_frame]
        z_q_4_predict = z_q_4[:, start_frame:mid_frame]
        ...
        for i in range(mid_frame, end_frame):
            ...
            z_q_1_ = self.transformer_1.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_1_predict,
                start_frame=start_frame, mid_frame=i)
            idx_1 = torch.argmax(z_q_1_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_1 = self.vae.vqvae.embed_code(idx_1).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12
            
            z_q_2_ = self.transformer_2.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_2_predict, guide=next_quant_1,
                start_frame=start_frame, mid_frame=i)
               
            idx_2 = torch.argmax(z_q_2_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_2 = self.vae.vqvae.embed_code(idx_2).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12

            z_q_3_ = self.transformer_3.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_3_predict, guide=next_quant_2,
                start_frame=start_frame, mid_frame=i)
               
            idx_3 = torch.argmax(z_q_3_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_3 = self.vae.vqvae.embed_code(idx_3).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12

            z_q_4_, rel_poses_b_ = self.transformer_4.forward_autoreg_step( # NOTE only FORECAST in this scale rough forecast and refine in upscaled features
                z_q_4_predict, guide=next_quant_3, pose_tokens=rel_poses_state,
                start_frame=start_frame, mid_frame=i)

            idx_4 = torch.argmax(z_q_4_.flatten(0, 1), dim=1) # bf c h w -> bf h w
            next_quant_4 = self.vae.vqvae.embed_code(idx_4).permute(0,3,1,2).view(*z_shape) # bf, 128, 12, 12
            
            z_q_1_list.append(z_q_1_[:, -1:])
            z_q_2_list.append(z_q_2_[:, -1:]) # NOTE ADDED
            z_q_3_list.append(z_q_3_[:, -1:])
            z_q_4_list.append(z_q_4_[:, -1:])
            ...
            z_q_1_predict = torch.cat([z_q_1_predict, next_quant_1[:, -1:]], dim=1)
            z_q_2_predict = torch.cat([z_q_2_predict, next_quant_2[:, -1:]], dim=1)
            z_q_3_predict = torch.cat([z_q_3_predict, next_quant_3[:, -1:]], dim=1)
            z_q_4_predict = torch.cat([z_q_4_predict, next_quant_4[:, -1:]], dim=1)
        ...
        z_q_2_predict = z_q_2_predict[:, mid_frame:end_frame] 
        z_q_3_predict = z_q_3_predict[:, mid_frame:end_frame] 
        z_q_4_predict = z_q_4_predict[:, mid_frame:end_frame] 
        ...
        z_q_1 = torch.cat(z_q_1_list, dim=1)
        z_q_2 = torch.cat(z_q_2_list, dim=1)
        z_q_3 = torch.cat(z_q_3_list, dim=1)
        z_q_4 = torch.cat(z_q_4_list, dim=1)
        # z_q_t = torch.cat(z_q_t_list, dim=1)
        # z_q_m = torch.cat(z_q_m_list, dim=1)
        # z_q_b = torch.cat(z_q_b_list, dim=1)
        output_dict['ce_inputs_1'] = z_q_1.flatten(0, 1)
        output_dict['ce_inputs_2'] = z_q_2.flatten(0, 1)
        output_dict['ce_inputs_3'] = z_q_3.flatten(0, 1)
        output_dict['ce_inputs_4'] = z_q_4.flatten(0, 1)
        ...
        quant_final = (z_q_1_predict + z_q_2_predict + z_q_3_predict + z_q_4_predict).flatten(0, 1).permute(0, 2, 3, 1)

        # NOTE TODO WONHYEOK: VAE decode function 확인필요 ->
        z_q_predict = self.vae.decode(quant_final, shapes, output_dict['target_occs'].shape)
        output_dict['logits'] = z_q_predict
        pred = z_q_predict.argmax(dim=-1).detach().cuda()
        output_dict['sem_pred'] = pred
        pred_iou = deepcopy(pred)
