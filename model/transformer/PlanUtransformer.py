...
class PlanUAutoRegTransformerResidual(BaseModule):
    def __init__(
        ...
        self.occ_attentions_en = nn.ModuleList([])
        self.occ_attentions_de = nn.ModuleList([])
        self.encoders = nn.ModuleList()
        self.decoders = nn.ModuleList()
        ...
        pre_c = C
        for channel, cH, cW in zip(channels[0:-1], Hs[0:-1], Ws[0:-1]):
            occ_attn_layer = nn.ModuleList()
            for i in range(temporal_attn_layers): # TODO DEPRECATED FEATURE: temporal -> occ
                if without_temporal_attn:
                    assert 0
            ...
            self.occ_attentions_en.append(occ_attn_layer)
            if without_spatial_attn:
                self.encoders.append(nn.Sequential(IdentityUnetBlock((cH, cW), pre_c, channel), 
                                                   IdentityUnetBlock((cH, cW), channel, channel)))
            else:
        ...
        for channel, cH, cW in zip(channels[-2::-1], Hs[-2::-1], Ws[-2::-1]):
            ...
            self.occ_attentions_de.append(occ_attn_layer)
            if without_spatial_attn:
                self.decoders.append(nn.Sequential(
                    IdentityUnetBlock((channel_agg, cH,cW), channel_agg, channel, True),
                    IdentityUnetBlock((channel, cH,cW), channel, channel, True)
        ...
        self.conv_out = nn.Conv2d(pre_c, output_channel, 3, 1, 1) # TODO: depth-wise convolution?
        ...
        if tokens_untouched:
            ...
            # NOTE TEMPORAL ATTENTION MASK
            attn_mask = torch.zeros(num_frames * num_tokens * 4, num_frames * num_tokens, dtype=torch.bool)
            for i_frame in range(num_frames):
    ...
    def forward(self, queries, tokens, pose_tokens):
        ...
        assert F == self.num_frames
        queries = rearrange(queries, 'b f c h w d -> b f d h w c')
        tokens = rearrange(tokens, 'b f c h w -> b f h w c')
        ...
        for occ_attn_en, encoder, down, pose_attn_en, pose_en in zip(self.occ_attentions_en, self.encoders, self.downsamples, self.pose_attn_en, self.pose_en):
            ...
            tokens = rearrange(tokens, 'b f c h w -> (b f) c h w')
            ...
            for temporal_attn, temporal_norm, spatial_attn, spatial_norm, depth_attn, depth_norm, ffn, ffn_norm in occ_attn_en:
                ...
                # pose_queries = rearrange(pose_queries, 'b f c -> (b f) 1 c')
                queries = rearrange(queries, '(b h w) (f d) c -> (b f) (h w d) c', b=b, h=h, w=w, d=d)
                pose_queries = rearrange(pose_queries, 'b f c -> (b f) 1 c')
                ...
                '''NOTE NOTE : Ablation) 마지막에 depth transformer로 대체'''
                ...
                queries = rearrange(queries, '(b f) (h w d) c -> (b f h w) d c', b=b, f=f, h=h, w=w, d=d)
                tokens = rearrange(tokens, '(b h w) f c -> (b f h w) 1 c', b=b, h=h, w=w)
            ...
            # tokens = rearrange(tokens, '(b f d) c h w -> b f d c h w', b=b, f=f, d=d)
            queries = rearrange(queries, '(b f d) c h w -> b f d c h w', b=b, f=f, d=d)
            tokens = rearrange(tokens, '(b f) c h w -> b f c h w', b=b, f=f)
        ...
        for occ_attn_de, decoder, up, encoder_out_queries, encoder_out_tokens, pose_attn_de, pose_de_, encoder_out_pose_queries, encoder_out_pose_tokens, pose_up in zip(self.occ_attentions_de,
                                                                        self.decoders, self.upsamples, encoder_outs_queries[::-1],
                                                                        encoder_outs_tokens[::-1], self.pose_attn_de, self.pose_de,
                                                                        encoder_outs_pose_queries[::-1], encoder_outs_pose_tokens[::-1], self.pose_up):
            queries = up(queries)
    ...
    def forward_autoreg_step(self, tokens, res_tokens, pose_tokens=None, start_frame=0, mid_frame=6):
        ...
        for occ_attn_en, encoder, down, pose_attn_en, pose_en in zip(self.occ_attentions_en, self.encoders, self.downsamples, self.pose_attn_en, self.pose_en):
            ...
            for temporal_attn, temporal_norm, spatial_attn, spatial_norm, depth_attn, depth_norm, ffn, ffn_norm in occ_attn_en:
                queries = rearrange(queries, '(b f d) c h w -> (b h w d) f c', b=b, f=f, d=d)
                tokens = rearrange(tokens, '(b f d) c h w -> (b h w d) f c', b=b, f=f, d=d)
                queries = queries + temporal_attn(queries, tokens, tokens, need_weights=False, attn_mask=self.attn_mask[:f, :f])[0]
                queries = temporal_norm(queries)
