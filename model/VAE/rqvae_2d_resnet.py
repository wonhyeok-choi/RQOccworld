...
class RQVAERes2D(BaseModule):
    def __init__(
        ...
        self.quant_conv = nn.Conv2d(128, 128, 1)
        self.post_quant_conv = nn.Conv2d(128, 128, 1)
    ...
    def forward_encoder(self, x):
        ...
        x = x.reshape(bs*F, H, W, D * self.expansion).permute(0, 3, 1, 2) # (10, 128, 200, 200)

        feat, shape = self.encoder(x) # [10, 128, 50, 50] // [[200, 200], [100, 100]]
        feat = self.quant_conv(feat)

        return feat, shape
        
    def forward_decoder(self, z, shapes, input_shape):
        feat = self.post_quant_conv(z.permute(0, 3, 1, 2))
        logits = self.decoder(feat, shapes) # [bs*F, 128, 50, 50] -> [bs*F, 128, 200, 200]
    ...
    def forward(self, x, **kwargs):
        ...
        quant, loss, _, _ = self.vqvae(feat) # 128, 12, 12
            
        logits = self.forward_decoder(quant, shapes, x.shape) # [bs, 10, 200, 200, 16, 18]
    ...
    @torch.no_grad()
    def encode_soft_code(self, x, temp, stochastic):
        feat, shapes = self.forward_encoder(x)
        residual_list, code_list, soft_code_list = self.vqvae.get_soft_codes(feat, temp=temp, stochastic=stochastic)
        return residual_list, code_list, soft_code_list, shapes, x.shape
    
    def decode(self, quant_sum, shapes, out_shape):
        logits = self.forward_decoder(quant_sum, shapes, out_shape) # [bs, 10, 200, 200, 16, 18]
        return logits
    
    def decode_code(self, code_list, shapes, out_shape):
        assert len(code_list) == 3
        final_quant = 0.0
        for code in code_list:
            final_quant += self.vqvae.embed_code(code)

        dec = self.decode(final_quant, shapes, out_shape)
        return dec

    # def get_code(self, code_t, code_m, code_b):
    #     quant_t = self.vqvae_top.embed_code(code_t)
    #     quant_m = self.vqvae_middle.embed_code(code_m)
    #     quant_b = self.vqvae_bottom.embed_code(code_b)

    #     return quant_t, quant_m, quant_b
