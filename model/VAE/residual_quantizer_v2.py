...
class ResidualVectorQuantizerV2(BaseModule):
    ...
    def forward(self, z, isSoft=False):
        z = rearrange(z, 'b c h w -> b h w c').contiguous()
        quant_list, residual_list, code_list, soft_code_list = self.quantize(z, isSoft)
        loss = self.compute_loss(z, quant_list)
        final_quant = z + (quant_list[-1] - z).detach()

        return final_quant, loss, residual_list, code_list, soft_code_list

    def quantize(self, z, isSoft=False):
        ...
        for i in range(self.depth): # hardcoded
            quantize, code, soft_code = self.codebooks[i](residual_feature, isSoft)
            # NOTE soft_code: b h w c
...
class VQEmbedding(nn.Embedding):
    ...
    @torch.no_grad()
    def find_nearest_embedding(self, inputs, isSoft=False):
        ...
        if isSoft:
            temp = 1.0
            soft_code = F.softmax(-distances / temp, dim=-1) # b h w c

            # soft_code_flat = soft_code.reshape(-1, soft_code.shape[-1]) # bhw c
            # soft_embed_idxs = torch.multinomial(soft_code_flat, 1) # 
            # soft_embed_idxs = soft_embed_idxs.reshape(*soft_code.shape[:-1]) # bhw

            return embed_idxs, soft_code

        return embed_idxs, None
    ...
    def forward(self, inputs, isSoft=False):
        embed_idxs, soft_embed_idxs = self.find_nearest_embedding(inputs, isSoft)
        # if self.training:
        #     assert 0
        #     if self.ema:
        ...
        return embeds, embed_idxs, soft_embed_idxs
